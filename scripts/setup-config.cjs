...
async function main() {
  ...
  const pagesName = await ask('Pages project name (default: bookmark-navigator-pages): ', 'bookmark-navigator-pages');
  const dbName = await ask('D1 database name (default: bookmark-navigator): ', 'bookmark-navigator');
  const dbId = await ask('D1 database id (leave blank for local-only template): ', '');
  const adminPassword = await ask('Admin password for .dev.vars (default: change-me-now): ', 'change-me-now');
  const jwtSecret = await ask('JWT secret for .dev.vars (default: random): ', generateSecret());
