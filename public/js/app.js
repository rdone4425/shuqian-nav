...
const App = {
  ...
  bindEvents() {
    ...
    document.addEventListener("click", (e) => {
      if (
        !this.elements.toolsMenuToggle?.contains(e.target) &&
    ...
    try {
      ...
      try {
        ...
        if (!allowedProtocols.includes(url.protocol)) {
          this.showMessage(
            `不支持的协议: ${url.protocol}。只允许 HTTP, HTTPS, FTP, FTPS`,
            "error",
          );
          return;
        }
        ...
        if (
          dangerousProtocols.some((protocol) =>
            formData.url.toLowerCase().startsWith(protocol),
          )
        ) {
  ...
  async saveAiSettings() {
    ...
    const submitBtn = this.elements.aiSettingsForm?.querySelector(
      'button[type="submit"]',
    );
...
`;
  ...
  async executeImport(data, modal) {
    ...
    try {
      ...
      if (response.success) {
        const { imported, skipped, errors, total, errorDetails } =
          response.data;
  ...
  setViewMode(mode) {
    ...
    if (bookmarksGrid) {
      bookmarksGrid.className =
        mode === "list" ? "bookmarks-list" : "bookmarks-grid";
    }
  },
