...
export async function onRequestPost(context) {
  ...
  try {
      ...
      try {
        ...
        try {
          ...
          if (!["http:", "https:"].includes(url.protocol)) {
            errorCount++;
            errors.push(
              `第 ${index + 1} 个书签: 不支持的URL协议 ${url.protocol}`,
            );
            continue;
          }
        } catch {
          errorCount++;
          errors.push(
            `第 ${index + 1} 个书签: 无效的URL格式 "${bookmark.url}"`,
          );
          continue;
        }
        ...
        errorCount++;
        errors.push(
          `处理书签失败: ${bookmark.title || bookmark.url} - ${error.message}`,
        );
    ...
    `,
      ...
      .bind("has_user_data", "true", "系统是否包含用户导入的数据")
      .run();

    console.log(
      `导入完成: ${importedCount} 成功, ${skippedCount} 跳过, ${errorCount} 失败`,
    );
