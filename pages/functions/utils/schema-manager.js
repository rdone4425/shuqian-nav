...
const SEED_STATEMENTS = [
  ...
  [
    `INSERT OR IGNORE INTO system_config (config_key, config_value, description)
     VALUES (?, ?, ?)`,
    ["site_title", "Bookmark Navigator", "Site title"],
  ],
  [
    `INSERT OR IGNORE INTO system_config (config_key, config_value, description)
     VALUES (?, ?, ?)`,
    ["site_description", "Bookmark management system", "Site description"],
  ],
  [
    `INSERT OR IGNORE INTO categories (name, color, description)
     VALUES (?, ?, ?)`,
    ["Tools", "#10B981", "Development tools and utilities"],
  ],
  [
    `INSERT OR IGNORE INTO categories (name, color, description)
     VALUES (?, ?, ?)`,
    ["Learning", "#3B82F6", "Learning resources"],
  ],
  [
    `INSERT OR IGNORE INTO categories (name, color, description)
     VALUES (?, ?, ?)`,
    ["Entertainment", "#F59E0B", "Entertainment and leisure"],
  ],
  [
    `INSERT OR IGNORE INTO categories (name, color, description)
     VALUES (?, ?, ?)`,
    ["Other", "#6B7280", "Uncategorized websites"],
  ],
];
...
export async function initializeDatabase(db) {
  for (const statement of [...SCHEMA_STATEMENTS, ...INDEX_STATEMENTS]) {
    await db.prepare(statement).run();
  }

  for (const [statement, params] of SEED_STATEMENTS) {
    await db
      .prepare(statement)
      .bind(...params)
      .run();
...
export async function getMissingCoreTables(db) {
  const missing = [];
