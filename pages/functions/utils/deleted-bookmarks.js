...
export async function insertDeletedBookmarkSafe(env, bookmark, options = {}) {
  try {
    ...
    `,
      ...
      .bind(bookmark.url, bookmark.id)
      .first();

    if (existingRecord) {
      console.log(
        `删除记录已存在 (URL: ${bookmark.url}, ID: ${bookmark.id})，跳过重复插入`,
      );
    ...
    if (result.success) {
      console.log(
        `成功插入删除记录 (URL: ${bookmark.url}, ID: ${bookmark.id})`,
      );
      return true; // 成功插入新记录
    } else {
 ...
 */
export async function insertDeletedBookmarksBatch(
  env,
  bookmarks,
  options = {},
) {
  ...
  try {
    ...
    for (const duplicate of duplicates.results || []) {
      ...
      `,
        ...
        .bind(duplicate.url, duplicate.original_bookmark_id)
        .all();
