...
export class BackupManager {
  ...
  async createFullBackup(env) {
    try {
      ...
      // 导出分类数据
      const categoriesResult = await env.BOOKMARKS_DB.prepare(
      ...
      // 导出已删除书签（如果表存在）
      try {
        const deletedResult = await env.BOOKMARKS_DB.prepare(
          "SELECT * FROM deleted_bookmarks ORDER BY deleted_at DESC LIMIT 1000",
        ).all();
  ...
  async createIncrementalBackup(env, lastBackupTime) {
    try {
      ...
      // 获取修改的分类
      const categoriesResult = await env.BOOKMARKS_DB.prepare(
        "SELECT * FROM categories WHERE updated_at > ? ORDER BY id",
      )
        .bind(lastBackupTime)
        .all();
  ...
  async uploadToR2(env, backup, filename) {
    ...
    try {
      ...
      const response = await env.BACKUP_BUCKET.put(filename, backupData, {
        httpMetadata: {
          contentType: this.compressionEnabled
            ? "application/gzip"
            : "application/json",
        ...
        for (const category of backupData.data.categories) {
          try {
            await env.BOOKMARKS_DB.prepare(
              "INSERT OR REPLACE INTO categories (id, name, color, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
            )
              .bind(
                category.id,
                category.name,
